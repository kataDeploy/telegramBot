...
function App() {
  ...
  const [backendTimeInterval, setBackendTimeInterval] = useState("4h");
  ...
  const setTelegramConfig = () => {
    ...
    axios.post(backendIp + path, { telegramApi, chatId, backendTimeInterval }).then((res) => {
        ...
        <div>
          ...
          <div>
            <div style={{ width: "300px", margin: "10px 10px" }}>
              ...
              ></Input>
              <Input
                placeholder="backend time interval"
                onChange={(e) => setBackendTimeInterval(e.target.value)}
              ></Input>
            </div>
            <Button
              className="setupButtons"
